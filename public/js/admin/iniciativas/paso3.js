        ...
        success: function (resListar) {
            ...
            if (infraestructura.length > 0) {
                infraestructura.forEach((registro) => {
                    if (registro.enti_codigo == 1)
                        totalEmpresa =
                            totalEmpresa +
                            parseInt(registro.suma_infraestructura);
                    else
                        totalExterno =
                            totalExterno +
                            parseInt(registro.suma_infraestructura);
            ...
            if (rrhh.length > 0) {
                rrhh.forEach((registro) => {
                    if (registro.enti_codigo == 1)
                        totalEmpresa =
                            totalEmpresa + parseInt(registro.suma_rrhh);
                    else
                        totalExterno =
                            totalExterno + parseInt(registro.suma_rrhh);
                });
            }
            $("#empresa-total").text(
                "$" +
            ...
            if (dinero.length > 0) {
                dinero.forEach((registro) => {
                    if (registro.enti_codigo == 1) {
                        ...
                        total_vcm_sede =
                            total_vcm_sede +
                            parseInt(registro.suma_dinero_vcm_sede);
                        total_vcm_escuela =
                            total_vcm_escuela +
                            parseInt(registro.suma_dinero_vcm_escuela);
                        total_vra =
                            total_vra + parseInt(registro.suma_dinero_vra);
                    } else {
                        totalExterno =
                            totalExterno + parseInt(registro.suma_dinero);
                    }
        ...
        success: function (resListar) {
            ...
            if (especies.length > 0) {
                especies.forEach((registro) => {
                    if (registro.enti_codigo == 1)
                        totalEmpresa =
                            totalEmpresa + parseInt(registro.suma_especies);
                    else
                        totalExterno =
                            totalExterno + parseInt(registro.suma_especies);
                });
            }
            $("#empresa-especies-total").text(
                "$" +
                    new Intl.NumberFormat("es-CL", {
                        maximumSignificantDigits: 13,
                    }).format(totalEmpresa)
            );
            $("#externo-especies-total").text(
                "$" +
                    new Intl.NumberFormat("es-CL", {
                        maximumSignificantDigits: 13,
                    }).format(totalExterno)
...
function cargarInfraestructura() {
    ...
    $.ajax({
        ...
        success: function (resListar) {
            ...
            $("#empresa-infra-total").text(
                "$" +
                    new Intl.NumberFormat("es-CL", {
                        maximumSignificantDigits: 13,
                    }).format(totalEmpresa)
            );
            $("#externo-infra-total").text(
                "$" +
                    new Intl.NumberFormat("es-CL", {
                        maximumSignificantDigits: 13,
                    }).format(totalExterno)
...
function cargarRrhh() {
    ...
    $.ajax({
        ...
        success: function (resListar) {
            ...
            $("#empresa-rrhh-total").text(
                "$" +
                    new Intl.NumberFormat("es-CL", {
                        maximumSignificantDigits: 13,
                    }).format(totalEmpresa)
            );
            $("#externo-rrhh-total").text(
                "$" +
                    new Intl.NumberFormat("es-CL", {
                        maximumSignificantDigits: 13,
                    }).format(totalExterno)
    ...
    $.ajax({
        ...
        success: function (resListar) {
            ...
            if (datosInfra.length == 0) {
                $("#codigoinfra").append(
                    new Option("No existen registros", "-1")
                );
            } else {
                datosInfra.forEach((registro) => {
                    $("#codigoinfra").append(
                        new Option(registro.tinf_nombre, registro.tinf_codigo)
                    );
                });
            }
        },
        ...
        success: function (resListar) {
            ...
            if (datosRrhh.length == 0) {
                ...
                datosRrhh.forEach((registro) => {
                    $("#codigorrhh").append(
                        new Option(registro.trrhh_nombre, registro.trrhh_codigo)
                    );
                });
            }
        },
    ...
    $.ajax({
        ...
        success: function (resConsultar) {
            ...
            if (respuesta.trrhh_codigo == trrhh_codigo) {
                if (corh_horas != "")
                    $("#valorrrhh").val(corh_horas * respuesta.trrhh_valor);
